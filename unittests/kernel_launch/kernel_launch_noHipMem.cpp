...
int main(int argc, char* argv[])
{
    ...
    for (int i=0; i<launches; i++) {
        if (!sharedMem) {
            ...
            hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel), dim3(BLOCKS), dim3(THREADS), 0, 0, input, output); 
        } else {
            //hipLaunchKernelGGL(HIP_KERNEL_NAME(shmKernel), dim3(BLOCKS), dim3(THREADS), 0, 0, dinput, doutput);
            hipLaunchKernelGGL(HIP_KERNEL_NAME(shmKernel), dim3(BLOCKS), dim3(THREADS), 0, 0, input, output);
        }
    }

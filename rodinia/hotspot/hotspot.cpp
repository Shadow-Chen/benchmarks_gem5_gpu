...
__global__ void calculate_temp(int iteration,  //number of iteration
	...
	__syncthreads();
        ...
        int validYmin = (blkY < 0) ? 0-blkY : 0;
        int validYmax = (blkYmax > grid_rows-1) ? BLOCK_SIZE-1-(blkYmax-grid_rows+1) : BLOCK_SIZE-1;
        int validXmin = (blkX < 0) ? 0-blkX : 0;
        ...
        for (int i=0; i<iteration ; i++){ 
            ...
            if( IN_RANGE(tx, i+1, BLOCK_SIZE-i-2) &&  \
                  ...
                  temp_t[ty][tx] =   temp_on_cuda[ty][tx] + step_div_Cap * (power_on_cuda[ty][tx] + 
	       	         (temp_on_cuda[S][tx] + temp_on_cuda[N][tx] - val*temp_on_cuda[ty][tx]) * Ry_1 + 
		             (temp_on_cuda[ty][E] + temp_on_cuda[ty][W] - val*temp_on_cuda[ty][tx]) * Rx_1 + (amb_temp - temp_on_cuda[ty][tx]) * Rz_1);
            }
            __syncthreads();
            if(i==iteration-1)
                break;
		...
		int total_iterations, int num_iterations, int blockCols, int blockRows, int borderCols, int borderRows) 
{
	...
	for (t = 0; t < total_iterations; t+=num_iterations) {
            ...
            src = dst;
            dst = temp;
            hipLaunchKernelGGL(HIP_KERNEL_NAME(calculate_temp), dim3(dimGrid), dim3(dimBlock), 0, 0, MIN(num_iterations, total_iterations-t), MatrixPower,MatrixTemp[src],MatrixTemp[dst],\
...
char dtfile[] = "/home/benchmarks/rodinia/hotspot/temp_64"; //data/temp.dat";
char dpfile[] = "/home/benchmarks/rodinia/hotspot/power_64"; //data/power.dat";
char dofile[] = "/home/benchmarks/rodinia/hotspot/out_64"; //output_pyramid.dat";

                ...
                int blockL, int border)
{
	...
	int bj = hipBlockIdx_x;
	int bk = hipBlockIdx_y/blockL;
	int bl = hipBlockIdx_y%blockL;
	int tj=hipThreadIdx_x;
	int tk=hipThreadIdx_y;
	int tl=hipThreadIdx_z;
              ...
              int border)
{
	...
	for (int t = 0; t < timesteps; t+=pyramid_height) {
            ...
            src = dst;
            dst = temp;
            hipLaunchKernelGGL(HIP_KERNEL_NAME(evolve), dim3(dimGrid), dim3(dimBlock), 0, 0, 
